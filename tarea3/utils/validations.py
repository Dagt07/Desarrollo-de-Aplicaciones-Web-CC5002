...
def validateDeporte(value):
    if value is None:
        return False
    deportes = [
        "Clavados", "Natación", "Natación artística", "Polo acuático", "Natación en Aguas abiertas",
        "Maratón", "Marcha", "Atletismo", "Bádminton", "Balonmano",
        "Básquetbol", "Básquetbol 3x3", "Béisbol", "Boxeo", "Bowling",
        "Breaking", "Canotaje Slalom", "Canotaje de velocidad", "BMX Freestyle", "BMX Racing",
        "Mountain Bike", "Ciclismo pista", "Ciclismo ruta", "Adiestramiento ecuestre", "Evento completo ecuestre",
        "Salto ecuestre", "Escalada deportiva", "Esgrima", "Esquí acuático y Wakeboard", "Fútbol",
        "Gimnasia artística Masculina", "Gimnasia artística Femenina", "Gimnasia rítmica", "Gimnasia trampolín", "Golf",
        "Hockey césped", "Judo", "Karate", "Levantamiento de pesas", "Lucha",
        "Patinaje artístico", "Skateboarding", "Patinaje velocidad", "Pelota vasca", "Pentatlón moderno",
        "Racquetball", "Remo", "Rugby 7", "Sóftbol", "Squash",
        "Surf", "Taekwondo", "Tenis", "Tenis de mesa", "Tiro",
        "Tiro con arco", "Triatlón", "Vela", "Vóleibol", "Vóleibol playa"]
    if value in deportes:
        return True
    return False
...
def validateTransporte(value):
    if value is None:
        return False
    transporte = ["particular", "locomoción pública"]
    if value in transporte:
        return True
    return False

# --- Validación en conjunto -------
def validate_register_artesan(nombre,email,telefono,region,comuna,descripcion,artesania,fotos):
...
def validate_register_hincha(nombre,email,telefono,region,comuna,comentario,deportes,transporte):
    if validateNombre(nombre) and validateEmail(email) and validatePhoneNumber(telefono) and validateDeporte(deportes) and\
    validateRegionYComuna(region,comuna) and validateComentarios(comentario) and validateTransporte(transporte):
        return True
    return False